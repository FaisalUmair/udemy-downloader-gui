    ...
    <el-form>
      ...
      <el-form-item class="text-center">
        <v-icon
          name="user-secret"
          scale="4"
          class="text-grey-darker border-4 border-grey-dark rounded-full py-4 px-5"
        ></v-icon>
      </el-form-item>
      ...
      <el-form-item>
        <el-button
          ...
          plain
          round
          @click="loginWithUdemy()"
        >Login Using Credentials</el-button>
      </el-form-item>
      ...
      <el-form-item>
        <el-button-group class="w-full">
          <el-button
            ...
            size="medium"
            class="w-1/2"
            @click="useAuthenticator()"
          >
            <v-icon name="user-secret" />Authenticator
          </el-button>
          <el-button
            type="danger"
            plain
            ...
            size="medium"
            class="w-1/2"
            @click="useAccessToken()"
          >
            <v-icon name="key" />Access Token
          </el-button>
        </el-button-group>
      </el-form-item>
    </el-form>
...
export default {
  ...
  created() {
    session.defaultSession.webRequest.onBeforeSendHeaders(
      { urls: ["*://*.udemy.com/*"] },
      (request, callback) => {
        if (request.requestHeaders.Authorization && !this.loggedIn) {
          settings.update(
            {},
            {
              $set: {
                access_token: request.requestHeaders.Authorization.split(" ")[1]
              }
            }
          );
