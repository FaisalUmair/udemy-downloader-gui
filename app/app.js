...
$(".ui.dashboard .content").on("click", ".load-more.button", function () {
  ...
  axios({
    ...
    .catch((error) => {
      appendLog(`loadMore_Error: ${error.response.status}`, error.message);
    })
    .finally(() => { activateBusy(false); });
...
function downloadButtonClick($course, subtitle) {
  ...
  axios({
    ...
    .then((response) => {
      ...
      $.each(resp.results, function (i, v) {
        ...
        if (v._class.toLowerCase() == "chapter") {
          ...
          if (v.asset.asset_type.toLowerCase() != "video" && downFiles == Settings.downloadType.OnlyLectures) {//skipAttachments) {
            remaining--;
            if (!remaining) {
          ...
          function getLecture(lecturename, chapterindex, lectureindex) {
            ...
            axios({
              ...
              .then((response) => {
                ...
                if (resp.supplementary_assets.length && downFiles != Settings.downloadType.OnlyLectures) { //!skipAttachments) {
                  ...
                  $.each(resp.supplementary_assets, function (a, b) {
                    ...
                    axios({
                      ...
                      .catch((error) => {
                        appendLog(`getLectureAndAttachments_Error: ${error.response.status}`, error.message);
              ...
              .catch((error) => {
                appendLog(`getLecture_Error: ${error.response.status}`, error.message);
                resetCourse($course, $course.find(".download-error"), false, coursedata);
              });
          }
        ...
        else if (downFiles != Settings.downloadType.OnlyLectures) { //(!skipAttachments) {
    ...
    .catch((error) => {
      ...
      appendLog(`download_Error: ${error.response.status}`, msgError);
    });
    ...
    try {
      ...
      function dlStart(dl, typeVideo, callback) {
              ...
              if (dl.status === -1
                ...
                axios({
                  ...
                  .catch((error) => {
                    ...
                    appendLog(`downloadLeacture_Error: ${statusCode}`, error.message);
        ...
        dl.on("error", function (dl) {
          console.error('errorDownload', dl);
          appendLog('errorDownload', dl.error.message);
          stopDownload();
          
          fs.unlink(dl.filePath, (err => {
            if (err) {
              appendLog('errorDownload', err);
              resetCourse($course, $course.find(".download-error"));
            }
            else {
              resetCourse($course, $course.find(".download-error"), Settings.download().autoRetry, coursedata, subtitle);
            }
      ...
      $download_quality.html(
        lectureQuality + (lectureType == "video" && !isNaN(parseFloat(lectureQuality)) ? "p" : "")
      ).removeClass(
        lastClass
      ).addClass(qualityColorMap[lectureQuality] || "grey");
      ...
      if (lectureType == "article" || lectureType == "url") {
        ...
        if (lectureQuality != "Highest") {
          ...
          if (fs.existsSync(seqName.fullPath + ".mtd") && !skipLecture) {
            ...
            console.log("downloadLecture: Iniciando download do Video ", coursedata["chapters"][chapterindex]["lectures"][lectureindex]["src"]);
...
$(".ui.settings .form").submit(e => {
  ...
  const checkNewVersion = $(e.target).find('input[name="check-new-version"]')[0].checked ?? true;
  const autoStartDownload = $(e.target).find('input[name="auto-start-download"]')[0].checked ?? false;
  const continueDonwloadingEncrypted = $(e.target).find('input[name="continue-downloading-encrypted"]')[0].checked ?? false;
  const enableDownloadStartEnd = $(e.target).find('input[name="enabledownloadstartend"]')[0].checked ?? false;
  const downFiles = $(e.target).find('input[name="downloadFiles"]:checked').val() ?? Settings.downloadType.LecturesAndAttachments;
  const skipSubtitles = $(e.target).find('input[name="skipsubtitles"]')[0].checked ?? false;
  const autoRetry = $(e.target).find('input[name="autoretry"]')[0].checked ?? false;
  const downloadStart = parseInt($(e.target).find('input[name="downloadstart"]').val()) ?? false;
  const downloadEnd = parseInt($(e.target).find('input[name="downloadend"]').val()) ?? false;
  const videoQuality = $(e.target).find('input[name="videoquality"]').val() ?? "Auto";
  const downloadPath = $(e.target).find('input[name="downloadpath"]').val() ?? false;
  const language = $(e.target).find('input[name="language"]').val() ?? false;
  const defaultSubtitle = $(e.target).find('input[name="defaultSubtitle"]').val() ?? false;
  const seqZeroLeft = $(e.target).find('input[name="seq-zero-left"]')[0].checked ?? false;
...
function loadSettings() {
  ...
  settingsForm.find('input[name="continue-downloading-encrypted"]').prop("checked", Settings.download().continueDonwloadingEncrypted ?? false);

  settingsForm.find('input[name="enabledownloadstartend"]').prop("checked", Settings.download().enableDownloadStartEnd ?? false);
  settingsForm.find('input[name="downloadstart"], input[name="downloadend"]').prop("readonly", Settings.download().enableDownloadStartEnd ?? false);
...
function getMsgChangeSearchMode() {
  ...
  msg += Settings.subscriber()
      ...
      msg += `<br/>${translate("If it's wrong, change the search mode and try again")}`
      msg += `<div class="ui fluid buttons"><button class='ui primary button change-search-mode' onclick='toggleSubscriber()'>${translate("Change search mode")}</button></div>`
  msg += "</p>"
  return msg;
}
...
function rendererDownloads() {
  ...
  const downloads = Settings.downloadedCourses()
  if (downloads) {
    downloads.forEach(function (course) {

      let $course = htmlCourseCard(course, true);
...
function search(keyword) {
  ...
  axios({
    ...
    .catch((error) => {
      ...
      appendLog(`search_Error: ${error.response.status}`, error.message);
    })
    .finally(() => { activateBusy(false); });
}
...
function askForSubtitle(availableSubs, initDownload, $course, coursedata, defaultSubtitle = "") {
  ...
  for (var key in availableSubs) {
    const subtitle = key.replace('[Auto]', '').replace(`[${translate("Auto")}]`, '').trim();
...
function checkLogin() {
  if (Settings.accessToken()) {
    ...
    axios({
      ...
      .finally(() => { console.log("login finish") });
