...
const icon = __dirname + '/assets/images/build/icon.png';
var headers;
...
var subDomain = settings.get('subdomain');
...
$('.access-token').click(()=>{
  userPrompt(translate('Please enter access_token which you got from chrome cookies'), translate('Access Token'), icon)
    .then(access_token => {
      ...
      $.ajax({
        ...
        error: function (err) {
          prompt.alert(translate('Incorrect Access-Token'));
        }
      });
    })
    .catch(err => {
...
$('.ui.login .form').submit((e) => {
  ...
  settings.set('subdomain', subDomain);
